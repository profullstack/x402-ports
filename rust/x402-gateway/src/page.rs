//! The sales page: what a refused crawler is shown, and what its operator reads.

use super::Offer;

const CSS: &str = "
:root{color-scheme:light dark;--fg:#1a1a1a;--bg:#fff;--mut:#666;--line:#e5e5e5;--code:#f4f4f4;--acc:#0a5}
@media(prefers-color-scheme:dark){:root{--fg:#eee;--bg:#111;--mut:#aaa;--line:#333;--code:#1c1c1c;--acc:#3c9}}
body{margin:0;background:var(--bg);color:var(--fg);font:16px/1.55 system-ui,-apple-system,Segoe UI,Roboto,sans-serif}
main{max-width:44rem;margin:0 auto;padding:2.5rem 1.25rem 4rem}
h1{font-size:1.8rem;line-height:1.2;margin:0 0 .5rem}h2{font-size:1.15rem;margin:2rem 0 .5rem}
p{margin:.5rem 0}.mut{color:var(--mut)}
pre{background:var(--code);border:1px solid var(--line);border-radius:6px;padding:.9rem 1rem;overflow-x:auto;font-size:.88rem;line-height:1.45}
code{font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:.92em}
ol,ul{padding-left:1.3rem}li{margin:.3rem 0}
.price{font-size:2.2rem;font-weight:700;color:var(--acc);margin:.25rem 0}
table{border-collapse:collapse;margin:.5rem 0;font-size:.95rem}td,th{border-bottom:1px solid var(--line);padding:.35rem .6rem;text-align:left}
footer{margin-top:3rem;color:var(--mut);font-size:.85rem}
";

#[derive(Clone, Debug)]
pub struct PageContext {
    pub days: i64,
    pub total: String,
    pub site_name: String,
    pub site_url: String,
    pub buy_url: String,
    pub price: String,
    pub minutes: i64,
    pub max_days: i64,
    pub header: String,
    pub enabled: bool,
    pub offer: Offer,
    pub training: Vec<String>,
    pub retrieval: Vec<String>,
    pub contact: String,
}

fn e(s: &str) -> String {
    s.replace('&', "&amp;")
        .replace('<', "&lt;")
        .replace('>', "&gt;")
        .replace('"', "&quot;")
}

pub fn window_for(minutes: i64) -> String {
    if minutes == 1440 {
        "one day".into()
    } else if minutes % 1440 == 0 {
        format!("{} days", minutes / 1440)
    } else if minutes == 60 {
        "one hour".into()
    } else if minutes % 60 == 0 {
        format!("{} hours", minutes / 60)
    } else {
        format!("{minutes} minutes")
    }
}

pub fn render_page(c: &PageContext) -> String {
    let window = window_for(c.minutes);
    let nets: Vec<&str> = c.offer.accepts.iter().map(|a| a.network.as_str()).collect();
    let networks = if nets.is_empty() {
        "Base, Polygon or Ethereum".to_string()
    } else {
        nets.join(", ")
    };
    let (amount, span) = if c.days > 1 {
        (c.total.clone(), format!("{} × {window}", c.days))
    } else {
        (c.price.clone(), window.clone())
    };
    let more = if c.days > 1 {
        format!("<p class=\"mut\">This offer is for {} days at {} a day. The plain page at <code>{}</code> quotes one.</p>", c.days, e(&c.price), e(&c.buy_url))
    } else {
        format!("<p class=\"mut\">Want longer? Add <code>?days=&lt;n&gt;</code> to this URL for an offer of up to {} days at {} a day, or simply pay a whole multiple of the price: the pass lasts as many days as you paid for.</p>", c.max_days, e(&c.price))
    };
    let off = if c.enabled {
        ""
    } else {
        "<p><strong>Payments are not switched on here yet.</strong> The offer below is empty until the operator configures a payout address, so for now this crawler is simply refused.</p>"
    };
    let touch = if c.contact.is_empty() {
        ", contact the site".to_string()
    } else {
        format!(", <a href=\"{}\">get in touch</a>", e(&c.contact))
    };
    let list = |v: &[String]| v.iter().map(|s| e(s)).collect::<Vec<_>>().join(", ");
    format!(
        r#"<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<meta name="robots" content="noindex">
<title>Crawl access · {name}</title>
<style>{css}</style>
</head>
<body>
<main>
<h1>Training crawlers pay for access here.</h1>
<p class="mut">People read <a href="{site}">{name}</a> free. So do search engines and the retrieval crawlers behind AI answers, because they send readers back. A crawler that copies pages into a training corpus sends nobody back, so it pays for the time it spends.</p>

<div class="price">{amount} <span class="mut" style="font-size:1rem;font-weight:400">for {span} of requests</span></div>
{more}
{off}

<h2>How it works</h2>
<ol>
  <li>Any page you fetch answers <code>402 Payment Required</code>. This page, fetched with <code>Accept: application/json</code>, returns the x402 offer: USDC, <code>exact</code> scheme, on {networks}.</li>
  <li>Sign the payment and retry with the proof in an <code>X-PAYMENT</code> header. The response is a JSON receipt carrying a pass.</li>
  <li>Send the pass in <code>{header}</code> on every request until it expires, {window} per day paid, so a proof for three times the price buys three. When it expires, buy another. The sale is the pass, not the page: fetch the page again with the pass.</li>
</ol>

<h2>Pay with the CoinPay CLI</h2>
<p>Settlement is by CoinPay: the buyer's USDC goes straight to the site's wallet and CoinPay's relayer pays the gas, so you need USDC and nothing else.</p>
<pre><code>npm install -g @profullstack/coinpay
coinpay x402 pay {buy} --output pass.json
# or a week at once:
coinpay x402 pay "{buy}?days=7" --output pass.json</code></pre>
<p>The command fetches this page, reads the offer, opens a browser tab to approve the payment with the CoinPay Wallet extension or any EIP-6963 wallet (MetaMask, Rabby, Coinbase Wallet), and writes the receipt to <code>pass.json</code>. Then:</p>
<pre><code>PASS=$(node -p "require('./pass.json').pass")
curl -H "{header}: $PASS" {site}/</code></pre>

<h2>Pay from your own x402 client</h2>
<pre><code>curl -sS -H "Accept: application/json" {buy}
# 402 with {{ "x402Version": 2, "accepts": [ ... ] }}
# sign an EIP-3009 transferWithAuthorization for one entry, then:
curl -sS -H "X-PAYMENT: &lt;base64 proof&gt;" {buy}
# 200 with {{ "ok": true, "pass": "cp_...", "expires_at": "...", "days": 1, "header": "{header}" }}</code></pre>
<p class="mut">The days a proof buys are read off the value it authorizes: a whole multiple of the one-day amount, up to {max_days}. <code>?days=&lt;n&gt;</code> only changes what the offer quotes, so a standard client that pays exactly what is asked gets <em>n</em> days.</p>
<p class="mut">The proof is x402 v2 in CoinPay's dialect: <code>{{ x402Version: 2, scheme: "exact", network: "&lt;CAIP-2&gt;", payload: {{ signature, authorization }} }}</code>, base64-encoded. A proof is single-use; retrying with the same one returns the same pass, not a second charge.</p>

<h2>Who pays and who does not</h2>
<table>
<tr><th>Charged</th><td>{training}</td></tr>
<tr><th>Free, named in robots.txt</th><td>{retrieval}</td></tr>
<tr><th>Free</th><td>Everyone else: people, Googlebot, Applebot, Bingbot and any crawler not on the first line.</td></tr>
</table>
<p class="mut">If your crawler is on the first line and you believe it should not be, or you want more than a day at a time{touch}.</p>

<footer>Sold over <a href="https://x402.org">x402</a>, settled by <a href="https://coinpayportal.com">CoinPay</a>. Served by x402-gateway for Rust.</footer>
</main>
</body>
</html>
"#,
        name = e(&c.site_name),
        css = CSS,
        site = e(&c.site_url),
        amount = e(&amount),
        span = e(&span),
        more = more,
        off = off,
        networks = e(&networks),
        header = e(&c.header),
        window = e(&window),
        buy = e(&c.buy_url),
        max_days = c.max_days,
        training = list(&c.training),
        retrieval = list(&c.retrieval),
        touch = touch,
    )
}
