...
CSS;
    ...
    public static function windowFor(int $minutes): string
    {
        if ($minutes === 1440) {
            return 'one day';
        }
    ...
    public static function render(array $c): string
    {
        ...
        $days = $c['days'] ?? 1;
        $window = self::windowFor($c['minutes']);
        $networks = implode(', ', array_map(fn ($a) => $a['network'], $c['offer']['accepts'] ?? []));
        if ($networks === '') {
        ...
        $more = $days > 1
            ? '<p class="mut">This offer is for ' . $days . ' days at ' . $e($c['price']) . ' a day. The plain page at <code>' . $e($c['buy_url']) . '</code> quotes one.</p>'
            : '<p class="mut">Want longer? Add <code>?days=&lt;n&gt;</code> to this URL for an offer of up to ' . $c['max_days'] . ' days at ' . $e($c['price']) . ' a day, or simply pay a whole multiple of the price: the pass lasts as many days as you paid for.</p>';
        ...
        $touch = ($c['contact'] ?? '') === '' ? ', contact the site' : ', <a href="' . $e($c['contact']) . '">get in touch</a>';
        $list = fn (array $l) => implode(', ', array_map($e, $l));
        $css = self::CSS;
        $name = $e($c['site_name']);
        $site = $e($c['site_url']);
        $buy = $e($c['buy_url']);
...
<ol>
  ...
  <li>Send the pass in <code>$header</code> on every request until it expires, $windowE per day paid, so a proof for three times the price buys three. When it expires, buy another. The sale is the pass, not the page: fetch the page again with the pass.</li>
</ol>
...
<footer>Sold over <a href="https://x402.org">x402</a>, settled by <a href="https://coinpayportal.com">CoinPay</a>. Served by x402-gateway for PHP.</footer>
</main>
</body>
</html>
