defmodule X402Gateway.Page do
  @moduledoc "The sales page: what a refused crawler is shown, and what its operator reads."
  ...
  def window_for(1440), do: "one day"
  def window_for(m) when rem(m, 1440) == 0, do: "#{div(m, 1440)} days"
  def window_for(60), do: "one hour"
  def window_for(m) when rem(m, 60) == 0, do: "#{div(m, 60)} hours"
  def window_for(m), do: "#{m} minutes"

  def render(c) do
    days = Map.get(c, :days, 1)
    window = window_for(c.minutes)

    networks =
      case Enum.map_join(c.offer["accepts"] || [], ", ", & &1["network"]) do
        "" -> "Base, Polygon or Ethereum"
        n -> n
      end

    {amount, span} = if days > 1, do: {c.total, "#{days} × #{window}"}, else: {c.price, window}

    more =
      if days > 1,
        do:
          ~s(<p class="mut">This offer is for #{days} days at #{e(c.price)} a day. The plain page at <code>#{e(c.buy_url)}</code> quotes one.</p>),
        else:
          ~s(<p class="mut">Want longer? Add <code>?days=&lt;n&gt;</code> to this URL for an offer of up to #{c.max_days} days at #{e(c.price)} a day, or simply pay a whole multiple of the price: the pass lasts as many days as you paid for.</p>)
    ...
    touch =
      if c.contact in [nil, ""],
        do: ", contact the site",
        else: ~s(, <a href="#{e(c.contact)}">get in touch</a>)
    ...
    <title>Crawl access · #{e(c.site_name)}</title>
    <style>#{@css}</style>
    </head>
    <body>
    <main>
    <h1>Training crawlers pay for access here.</h1>
    <p class="mut">People read <a href="#{e(c.site_url)}">#{e(c.site_name)}</a> free. So do search engines and the retrieval crawlers behind AI answers, because they send readers back. A crawler that copies pages into a training corpus sends nobody back, so it pays for the time it spends.</p>

    <div class="price">#{e(amount)} <span class="mut" style="font-size:1rem;font-weight:400">for #{e(span)} of requests</span></div>
    #{more}
    #{off}

    <h2>How it works</h2>
    <ol>
      <li>Any page you fetch answers <code>402 Payment Required</code>. This page, fetched with <code>Accept: application/json</code>, returns the x402 offer: USDC, <code>exact</code> scheme, on #{e(networks)}.</li>
      ...
      <li>Send the pass in <code>#{e(c.header)}</code> on every request until it expires, #{e(window)} per day paid, so a proof for three times the price buys three. When it expires, buy another. The sale is the pass, not the page: fetch the page again with the pass.</li>
    </ol>
    ...
    coinpay x402 pay #{e(c.buy_url)} --output pass.json
    # or a week at once:
    coinpay x402 pay "#{e(c.buy_url)}?days=7" --output pass.json</code></pre>
    ...
    curl -H "#{e(c.header)}: $PASS" #{e(c.site_url)}/</code></pre>

    <h2>Pay from your own x402 client</h2>
    <pre><code>curl -sS -H "Accept: application/json" #{e(c.buy_url)}
    # 402 with { "x402Version": 2, "accepts": [ ... ] }
    # sign an EIP-3009 transferWithAuthorization for one entry, then:
    curl -sS -H "X-PAYMENT: &lt;base64 proof&gt;" #{e(c.buy_url)}
    # 200 with { "ok": true, "pass": "cp_...", "expires_at": "...", "days": 1, "header": "#{e(c.header)}" }</code></pre>
    ...
    <table>
    <tr><th>Charged</th><td>#{Enum.map_join(c.training, ", ", &e/1)}</td></tr>
    <tr><th>Free, named in robots.txt</th><td>#{Enum.map_join(c.retrieval, ", ", &e/1)}</td></tr>
    ...
    <footer>Sold over <a href="https://x402.org">x402</a>, settled by <a href="https://coinpayportal.com">CoinPay</a>. Served by x402-gateway for Elixir.</footer>
    </main>
    </body>
    </html>
    """
